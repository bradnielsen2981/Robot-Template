...
from hiwonderinterface import HiwonderInterface
...
class RobotInterface(HiwonderInterface):
    ...
    # Multiple detection types can be included: sonar, line, colour, model. (Model has not been implemented)
    # Confirmlevel is the number of unique detection types required before a stop.
    def move_direction_until_detection(self, movetype="forward", distanceto=250, detection_types=['sonar'], colour='red', colourshift=False, timelimit=5, confirmlevel=1):
        ...
        if movetype != None:
            if movetype == "forward":
                ...
                self.move_direction(power=38, direction=90, rotationspeed=0.07)
        ...
        while ((time.time() < endtime) and (self.command == "move_direction_until_detection")):
            ...
            #read in the camera detection data
            data.update(self.CAMERA.get_detection_data()) #update the values in the dictionary
            time.sleep(0.05)
            ...
            if 'colour' in detection_types:
                ...
                if 'found' in data['detect_colour'][colour]:
                    ...
                    if 'colour' not in detections:
                        ...
                        if len(detections) == confirmlevel:
                            break
                if colourshift: #if colour shift is on, shift the colour each second
                    count = int(time.time() - data['starttime'])%3
                    colour = colour_list[count]
                    self.CAMERA.set_detection_colour(colour)
    ...
    def rotate_arm_until_colour_detected(self, colour="red", timelimit=10):
        ...
        self.CAMERA.add_detection_task("detect_colour")
        self.CAMERA.set_detection_colour(colour)
        self.set_sonarLED_color(colour)
        endtime = time.time() + timelimit
    ...
    def was_object_pickup_successful(self, colour='red', timelimit=3):
        ...
        while ((time.time() < endtime) and (self.command == "was_object_pickup_successful")):
            ...
            if 'found' in data['detect_colour'][colour]:
                ...
                x, y = center
                deltaY = 480-y
                if ((deltaY < 100) and (width > 320) and (width < 400) and (abs(angle) < 5)):
                    data['success'] = True
                    break
        self.stop()
        self.CAMERA.clear_detection_tasks()
    ...
    # Code to test camera
    def test_camera(self, colour='red', timelimit=20, detect_line=False, colourshift=True, detect_colour=True):
        if detect_line:
            self.CAMERA.add_detection_task('detect_line') #line detecton is slow
        if detect_colour:
            self.CAMERA.add_detection_task('detect_colour')
            self.CAMERA.set_detection_colour(colour)
        colour_list = ["red","green","blue","black"]
        time.sleep(1)
        starttime = time.time()
        ...
        while time.time() < endtime:
            if not self.show_camera_window():
                break
            data = self.CAMERA.get_detection_data()
            time.sleep(0.05)
            ...
            if detect_colour:
                if 'found' in data['detect_colour'][colour]:
                    ...
                    deltaY = 480-y
                    deltaX = 320-x
                    print("Colour detected:",colour)
                    print("Center of Rect:",center)
                    print("Size of Rect:",size)
                    print("Angle of Rect:",angle)
                
                if colourshift: #if colour shift is on, shift the colour each second
                    count = int(time.time() - starttime)%len(colour_list)
                    colour = colour_list[count]
                    self.CAMERA.set_detection_colour(colour)
...
if __name__ == '__main__':
    ...
    data = ROBOT.move_direction_until_detection(movetype="turnleft", distanceto=250, detection_types=['colour'],
                                                colourshift=True, timelimit=5, confirmlevel=1)
    #find which colour was detected
    ...
    if colour_detected:
        ...
        data = ROBOT.move_toward_colour_detected(colour=colour)
        ROBOT.look_down()
        data = ROBOT.move_toward_colour_detected(colour=colour) #get closer
        data = ROBOT.rotate_arm_until_colour_detected_is_centered(colour=colour)
        data = ROBOT.pick_up_centered_object_with_look_down(data['y'])
        data = ROBOT.was_object_pickup_successful(colour=colour)
        if data['success'] == True:
            ROBOT.SOUND.say("Pick up successful")
            ROBOT.look_up()
            data = ROBOT.move_direction_until_detection(movetype="turnright", distanceto=250, detection_types=['colour'], colour="black", timelimit=5, confirmlevel=1)
